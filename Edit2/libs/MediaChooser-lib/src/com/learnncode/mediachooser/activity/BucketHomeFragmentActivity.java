...
/**
 * 选择图片的主页面Activity，图片按相册分类进行选择
 * @author Administrator
 *
 */
public class BucketHomeFragmentActivity extends ActionBarActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (MediaChooserConstants.showVideo) {
			mTabHost.addTab(mTabHost.newTabSpec("tab2").setIndicator("Video"),
					BucketVideoFragment.class, null);
		}

		if (MediaChooserConstants.showImage) {
			mTabHost.addTab(mTabHost.newTabSpec("tab1").setIndicator("Image"),
		...
		mTabHost.setOnTabChangedListener(new OnTabChangeListener() {
			...
			public void onTabChanged(String tabId) {
				...
				/*
				 * if(tabId.equalsIgnoreCase("tab1")){ if(imageFragment !=
				 * null){ if(videoFragment != null){
				 * fragmentTransaction.hide(videoFragment); }
				 * fragmentTransaction.show(imageFragment); } }else{
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		cameraMenuItem = menu
				.findItem(R.id.cameraImageViewFromMediaChooserHeaderBar);
