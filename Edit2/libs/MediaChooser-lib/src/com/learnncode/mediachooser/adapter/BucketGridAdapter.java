...
public class BucketGridAdapter extends ArrayAdapter<BucketEntry> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		imageParams.width  = mWidth/culumNum;
		imageParams.height = mWidth/culumNum;

		holder.imageView.setLayoutParams(imageParams);

		if(mIsFromVideo){
			new VideoLoadAsync(bucketVideoFragment, holder.imageView, false, mWidth/culumNum).executeOnExecutor(MediaAsync.THREAD_POOL_EXECUTOR, mBucketEntryList.get(position).bucketUrl.toString());

		}else{
			ImageLoadAsync loadAsync = new ImageLoadAsync(mContext, holder.imageView, mWidth/culumNum);
